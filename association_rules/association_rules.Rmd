...
```{r binning-data, fig.height=3, fig.width=5, message=FALSE}
...
```{r}
#create final dataset based on findings
...
rules1 <- apriori(final_assoc_data, parameter = list(minlen=2, maxlen=5, conf = .923, supp = .189),
...
rules2 <- apriori(final_assoc_data, parameter = list(minlen=2, maxlen=5, conf = .817, supp = .031),
...
plot(rules1, method="matrix", shading=c("support", "confidence"), main = "Republican Association Rules")
plot(rules2, method="matrix", shading=c("support", "confidence"), main = "Democratic Association Rules")
```
